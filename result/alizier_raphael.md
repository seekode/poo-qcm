...
**Nom:** _ALIZIER Raphaël__________________  
...
Il n'y aura qu'une seule instance de la classe DatabaseConnection car cette dernière est "static" est peut donc être appelée sans être instancée car appartenant à la classe donc unique.
De plus, le constructeur est privé car cette instance est "private".
...
`abstract` sert à définir une classe abstraite qui sera héritée par d'autre classe .
_______________________________________________________________________________
`extends` sert à définir une classe enfant qui récuperera les méthodes de la classe parent.
_______________________________________________________________________________
`super` sert à accéder aux membres de la classe parente depuis une classe enfant.
_______________________________________________________________________________
`@Override` sert à redéfinir une méthode parent.
_______________________________________________________________________________
Si on essaye de faire `new Animal("Test")`, le programme renverra "Je suis Test"  
_______________________________________________________________________________
...
Une interface permet de définir les méthodes que les classe enfant vont devoir utiliser. 
_______________________________________________________________________________
On utilise `implements` pour appeler l'interface dans une classe.
_______________________________________________________________________________
...
`nombreInstances` est static, il n'est donc pas nécessaire d'instancier la classe quand on l'appel alors que `valeur` est dynamique, il faut donc instancier la classe à chaque appel, d'où l'utilisation de `this`.
_______________________________________________________________________________
A chaque instance, getValeur() sera égale à 1 à chaque instance car redéfini à 0 à chaque instance tandis que getNombreInstance() sera égale à 1 puis 2 puis 3
_______________________________________________________________________________
