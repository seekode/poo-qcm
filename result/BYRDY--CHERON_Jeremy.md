...
- [x] C) Creer automatiquement des copies d'objets
...
- [x] C) Une classe ne peut implementer qu'une seule interface
...
Le constructeur est privé car on ne veut pas que n'importe qui puisse le modifier et par conséquent changer la base de données. On s'assure qu'il n'y aura qu'une seule instance de la classe DatabaseConnection qui sera créée grâce à getInstance() qui ne crée une nouvelle DatabaseConnection() que si l'instance n'existe pas, si elle existe, on la renvoie simplement ce qui évite d'avoir une instance pour chaque classe qui extend de DatabaseConnection.
...
Le mot abstract permet de créer des méthodes (fonctions) sans leur donner d'ordres si ce n'est les éléments qu'elles acceptent et le type de ce qu'elles retournent. C'est notamment le cas avec la fonction parler() présente dans Animal dont on sait uniquement qu'elle ne va rien renvoyer (void) et qu'elle ne prend aucun élément en paramètre. Le extends permet à la classe Chien d'accéder à tout ce qui est présent dans la classe abstraite Animal. Le `super` permet de dire que la variable nom déclarée dans la classe abstraite Animal prend la valeur du nom fournie en argument dans le constructeur. Et le `@Override` permet de réécrire la fonction parler déclarée dans la classe abstraite Animal, en lui disant notamment que désormais elle affiche le cri de l'animal.Si nous faisons `new Animal("Test")`, cela nous mettra une erreur car il n'est pas possible de créer une instance d'une classe abstraite.
...
Une interface permet de regrouper tous les éléments communs sans pour autant expliquer leur fonctionnement qui peut diverger. Par exemple ici, un véhicule peut démarrer, s'arrêter ou encore avoir une vitesse maximale mais ce ne seront pas les mêmes actions pour une moto que pour une voiture par exemple. On utilise `implements` pour pouvoir accéder aux méthodes demarrer(), arreter() et getVitesseMax(). Si la classe voiture n'implémente pas toutes les méthodes de l'interface, il y aura une erreur demandant que toutes les méthodes de l'interface Vehicule soient implémentées dans la classe Voiture.
...
`nombreInstances` est statique, c'est à dire qu'il aura les mêmes valeurs peu importe dans quelle classe nous nous trouvons. Et `valeur` sera modifié car il est dynamique est propre à la classe dans laquelle il se trouve. getNombreInstances() vaudra à chaque fois 3 et getValeur() vaudra 1 pour chaque instance.
...
- Classe 1 : [Livre] - Attributs : type, genre, auteur, titre, tome, langue - Role : C'est une classe abstraite qui contient simplement tout ce qui permet de définir un livre ainsi que la façon de le lire (comme un roman, comme un manga, etc)
- Classe 2 : [ListeDesLivres] - Attributs : livresPrésents, statut - Role : Elle servira à stocker tous les livres appartenant à la bibliothèque et de savoir s'ils sont empruntés ou non.
- Classe 3 : [Utilisateur] - Attributs : nom, empruntés - Role : Cette classe gère les utilisateurs afin de savoir quel utilisateur a emprunté quel livre
- Relation : ListeDesLivres et Utilisateur hérient tous les deux de Livre.
