...
- [x] C) La possibilite de creer des variables de plusieurs types
...
- [x] A) Remplacer une methode heritee par une nouvelle implementation
...
le constructeur est privé pour qu'il ne puisse pas être utilisé hors de la classe

la façon dont ce code garantit une instance unique de la classe est en vérifiant

si la variable statique "instance" est null, si elle est alors un creé une instance
...
"abstract" permet de rendre un classe ou une méthode abstraite, "extends" permet

de faire hériter une classe d'une autre, "super" appele le constructeur de la classe

parente et "@override" permet de redéfinir une méthode.

quand on fait new "Animal("Test")", on initialise la variable "nom" de la classe

"Aminal" et on lui donne la méthode "presenter"
...
une interface est un contrat que les classes implémentantes doivent respecter

le mot clé "implements" permet d'implémenter une interface, si une classe 

n'implemente pas toutes les methodes de l'interface, le programme cessera de
...
"nombreInstances" est initialisé uniquement au moment au le code est lu, dans la classe 

"valeur" est initialisé à chaque instance de "Compteur", dans l'instance elle-même.

Si on cree 3 instances de "Compteur" et qu'on appelle "incrementer()" une fois

sur chacune, la valeur de "getNombreInstances()" sera 3 et la valeur de "getValeur()"

sera 1 pour chaque instance
...
class1: book - attributes: author, title, classification, status - role: permet

de savoir quelle livre est disponible

class2: user - attributes: name, book_taken - role: permet de savoir qui a emprenter

un livre et le quel

class3: author - book_written - role: permet de savoir qui a écrit un/plusieurs

livre(s) et le(s) quel(s)

relation: "author" hérite de "user"
