...
Une interface permet de faire en sorte que chaque classes qui l'implemente ai les methodes qui sont déclarer par l'interface par exemple dans le cas suivant l'interface Vehicule dit qu'il doit y avoir une méthode demarrer, arreter, getVitesseMax. Donc toutes les classes qui implemente cette inteface devront avoir c'est methodes. On utilise "implements" pour dire que la class Voiture implemente l'interface Vehicule et donc qu'elle doit avoir les methodes requise par l'interface Vehicule. Si la classe Voiture n'implemente pas toutes les methodes de l'interface Vehicule alors on aura une erreur car pour que la classe voiture soit correcte il faut toutes les methodes de l'interface Vehicule.
...
"nombreInstances" est statique elle est donc partagé par toutes les instance. Par contre "valeur" est dynamique donc chaque instance a une copie differente.
Si on cree 3 instances de Compteur et qu'on appelle incrementer une fois sur chacune la valeur de getNombreInstances sera 3 et la valeur de getValeur sera 1.
...
- Classe 1 : Book - Attributs : id, title, author, genre - Role : Un livre de la bibliotheque
- Classe 2 : Client - Attributs : id, firstName, lastName - Role : Un client de la bibliotheque
- Classe 3 : Checkout - Attributs : book, client, checkoutDate, returnDate - Role : Gère les prêt des livres de la bibliotheque
- Relation : Checkout contient des attributs provenant des instances de Book et de Client
