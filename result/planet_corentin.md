...
Le constructeur est prive parce que il est appelé dans la classe elle même et ce n'est lui qui initialise la classe, ce code garantit qu'il n'y a qu'une seule instance de la classe DatabaseConnection car l'objet n'est créer seulement si l n'existait pas précedemment
_______________________________________________________________________________
...
le role de `abstract` est de définir une classe abstraite qui sera hérité par d'autre classes et qui définit et prépare des méthodes pour ces autres classes.
_______________________________________________________________________________

le role de `extends` est d'hérité de la classe abstrait parente et d'avoir les fonctions définis dans cette dernière déjà préparé.
_______________________________________________________________________________

le role de `super` est de permettre un accès entre la classe enfant et la classe parente afin d'envoyer les paramètre de cette dernière.
_______________________________________________________________________________

le role de `@Override` est de redéfinir la méthode qui à été préparé et hérité par la classe parente.
_______________________________________________________________________________

Si nous essayons de faire `new Animal("Test")`, nous rencontrerons une erreur car une classe abstraite ne peut pas être appelé telle quelle, il faut passer par ses classes enfants.
_______________________________________________________________________________
...
Une interface permet uniquement de définir des signatures, c'est à dire de préparer des fonctions et de leur donnés des noms et des paramètres
_______________________________________________________________________________

On utilise `implements` 
_______________________________________________________________________________

Si la classe `Voiture` n'implemente pas toutes les méthodes de l'interface, le programme rencontrera un problème car toutes les fonctions de l'interface doivent être implémentes et sont donc nécessaire au fonctionnement de l'objet
_______________________________________________________________________________
...
la difference entre `nombreInstances` et `valeur` est que `nombreInstances` n'est pas réinitialiser à chaque création d'objet et est plutot stocké dans la Classe au lieu de l'objet.
_______________________________________________________________________________

Si nous créons 3 instances de `Compteur` et que nous appellons `incrementer()` une fois pour chaque instance,
la valeur de `getNombreInstances()` sera de 3 pour chaques instance et la valeur de `getValeur()` sera de 1 pour chaque instance car `getNombreInstances` réfère à une variable stocké dans la Classe alors que `getValeur` réfère à une variable stocké dans l'objet
_______________________________________________________________________________
...
Classe 1 : [Book] - Attributs : String title; String author; Role : Livre correspondant au codeDouet dans la bibilothèque
_______________________________________________________________________________

Classe 2 : [Shelf] - Attributs : int codeDouet; int shelfNumber; Role : position dans la bibliothèque des livres correspondant a ce code de Douet
_______________________________________________________________________________

Classe 3 : [Douet]  Role : contient fonction de tri de désignation de Douet
_______________________________________________________________________________

Relation : [Book] extends [Shelf], [Shelf] extends [Douet]
_______________________________________________________________________________
